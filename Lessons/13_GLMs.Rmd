...
#All are significant
#Intercept is Paul Lake at depth 0
#Nitrogren increases with depth for Paul lake, but a decrease in nitrogren with depth for Peter lake

TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlim(0, 10)+
  labs(x = "Depth (m)", y = expression(Total_Nitrogren ~ (mu*g / L)), color="")+
...
# Marginal R squared is the fixed effect
# Conditional R squared is the variability explained when you include random effects, so model fits better
...
# Is there a significant difference between the model with the random effect and the model without the random effect
anova(TempTest.mixed, TempTest.fixed)
# The lower the AIC, the better.
# The p-value tells us whether those models have a significantly different fit
# The model with random effect is better in this case
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  ...
  theme(legend.spacing.x = unit(0, "cm")) #Make labels closer together in the legend
print(NTL.tempmodel)
...
> ANSWER: For each year, we see that there is a significant increase in surface temperature by 0.06 degrees per year. That means in 30 years, the lake will be (0.06 * 30) 1.8 degrees warmer.
