...
# Needs a timeseries object, not a data frame
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic")

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed)

# Data is the raw data plotted
# Seasonal is the seasonal variation from year to year based on the 365 day year
# Trend is an average across the years
# Remainder represents remainder + seasonal + trend which equals data
# Boxes on the right show relative difference in the scale of the y-axis
...
# Careful here with negative values coming up
# The STL function does not care about negative values, so you need to be aware of this
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
...
# Tests for normality
# P value less than 0.05, data are NOT normal
shapiro.test(EnoDischarge.early.subsample$discharge.mean)
shapiro.test(EnoDischarge.late.subsample$discharge.mean)

# Tests for variance
# The variance is significantly different between the two variables because of low p value
var.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean)

# Significant difference in discharge from one variable to the next
...
> Data are not normal, and their variance is significantly different from one another. Wilcox Test shows that the two are significantly different from one another. There's no information about which variable is higher. Use summary() for that

> Mean daily discharge in the Eno River from 1927-1971 was significantly higher than mean daily discharge from 1985-2019 (Wilcox Test, W = 1.17*10^8, p < 0.0001)
...
> aggregation: Taking multiple data points and creating one data point from them. For example, if you have daily data, you can get mean or sum for months or years.

> subsampling: Subsetting the data to include a certain portion or a random sample of your data points.

> interpolation: Predicting values based on existing data. Can fill in data gaps.
...
print(NCOzone)
#Interpolation not a great option here
...
#Some points are added/ filled in using linear interpolation
...
#Create a date column by combining the month, year, and day columns and separate it by a '-'
#Assumes its the first of the month since there are no values for day
#Date column with day month and year needed for timeseries
...
# Frequency = number of months in year
...
#No significant trend over time, accept null hypothesis with high p value
...
> There is no significant difference between the trends in monthly discharge data over the past X years (z= -1.06, p = 0.29) 
