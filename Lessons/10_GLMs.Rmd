...
# Null hypothesis is that the data ARE normal
shapiro.test((EPAair.subsample$Ozone)) #Can only test 5000 observations
#Significant p-value here, reject the null- data are NOT normal
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
#Right-skewed, bimodel 

qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)
#Data do not follow the normal qqline

#Is the majority of air quality values less than 50
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  #geom_density(stat = "count", fill = "gray") + #This counts the number of observations rather than proportions which is seen in the density plot
  geom_density(fill = "gray") +
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #Axes start at zero
print(Ozone.plot)
```
...
> Here we are testing whether or not the average ozone concentration is below 50 in the EPA Air dataset. We used a t-test to determine with statistical significance that the mean average ozone concentration is below 50 (p<0.0002). 
...
var.test(EPAair$Ozone ~ EPAair$Year) #significant p-value shows that the variances are not equal
...
O3.twosample #Reject null hypoethesis that the means are equal
O3.twosample$p.value

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year)
summary(O3.twosample2) #For each increase in year, Ozone will increase by 0.8452

par(mfrow = c(2,2))
plot(O3.twosample2)
#Residuals vs fitted
#Normal QQ- shows the fit of the model, model residuals not normally distributed
#Scale-Location- you want the line to be flat
#Residuals vs Leverage- you want the points to be within cooks distance
```
...
O3.twosample.wilcox
#Only if assumptions are violated for a t-test and you're not comfortable running the t-test
```
...
OzoneDensPlot <- ggplot(EPAair, aes(x=Ozone, fill=Year)) + 
  geom_density(alpha=.5)+
  scale_fill_manual(values=c("red","blue"))+
  labs(y="Density")
print(OzoneDensPlot)

OzoneBoxPlot<- ggplot(EPAair, aes(x=Year, y=Ozone))+
