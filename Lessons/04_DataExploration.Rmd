...
```{r, message = FALSE} 
# Message = FALSE will not print the message, such as the one for tidyverse package, which will cause errors when you try to knit
# Load package
library(tidyverse)
```
...
#To copy a pathname from a file on your computer, right click on the file, then press the option key and click copy pathname
```
...
> ANSWER: This file is located in a metadata folder with a file title that links with the csv data such as "USGS".
