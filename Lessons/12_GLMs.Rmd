...
getwd()
library(tidyverse)
options(scipen = 4) #Option to set R to display 4 digits
...
#Items to interpret: For each additional unit increase in depth, irradiance decreases by 95.89 units. At a depth of 0, irradeance is 486.818. Look at depth p-value as key p-value to tell us that irradeance is significantly impacted by depth. Adjusted R square says what percent of irradeance variability is explained by depth. R squared doesn't give you direction, you need correlation test for that.
...
>  Irradeance decreases significantly with increasing depth (linear regression, R2 = 0.31, df = 15,449, p < 0.0001)
...
#Residuals vs Fitted, points are clustered, not good
#Normal QQ, one big outlier
#Scale-Location, not a horizontal line, points are clustered
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #Cannot log things with a value of zero, will return an error
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) #log transform to make data look linear
summary(irradiance.regression2) #Higher R-squared, good! Relationship is better represented by the model
#Always look at adjusted R-squared, not multiple R squared
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") + #plots two variables against each other with a line of best fit
  scale_y_log10() + #Puts log values for y axis
  geom_point() 
print(irradiancebydepth2) 

# SE can also be removed
# SE FROM MODEL, not confidence interval from the data
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
...
``` {r, fig.height = 3, fig.width = 4}
#You can subset data within the lm
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                   tp_ug ~ depth + dissolvedOxygen)
summary(TPregression)

#With two independent variables, intercept is when both variables = 0
#Depth and oxygen are significant predictors of total phosphorus
#R squared shows a significant amount of variation being explained by the model
...
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #Remove repeat values
```
...
step(TPAIC) #Runs series of AIC tests to determine best model
#The last model printed is the most parsimonious (best) model
